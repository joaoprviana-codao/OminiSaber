(() => {
  ...
  const createCurriculumImport=async(payload,items)=>{await guard();const hash=await hashCurriculumFile(payload.file);const record={nome_arquivo:payload.file.name,arquivo_hash_sha256:hash,origem:payload.origem||null,ano_letivo:payload.ano?Number(payload.ano):null,materia_codigo:payload.materia||null,trimestre:payload.trimestre?Number(payload.trimestre):null,status:'revisao',resumo:payload.resumo||{},documento_texto_extraido:payload.texto||null};const{data,error}=await db.from('importacoes_curriculo').insert(record).select().single();if(error&&error.code==='23505'){const existing=await db.from('importacoes_curriculo').select('*,importacoes_curriculo_itens(*)').eq('arquivo_hash_sha256',hash).single();return{duplicate:true,importacao:existing.data,items:existing.data?.importacoes_curriculo_itens||[]}}if(error)throw error;const staged=(items||[]).map(item=>({...item,importacao_id:data.id}));let stagedItems=[];if(staged.length){const result=await db.from('importacoes_curriculo_itens').insert(staged).select();if(result.error)throw result.error;stagedItems=result.data||[]}return{duplicate:false,importacao:data,items:stagedItems}};
  ...
  const updateCurriculumImportItem=async(id,changes)=>{await guard();const{data,error}=await db.from('importacoes_curriculo_itens').update(changes).eq('id',id).select().single();if(error)throw error;return data};
