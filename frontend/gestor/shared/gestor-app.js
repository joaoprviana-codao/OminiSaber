(() => {
      ...
      (b) =>
        (b.onclick = async () => {
          const x = rows.find((r) => r.id === b.dataset.id),
            supportsActive = Object.prototype.hasOwnProperty.call(x, "ativo");
          const body =
            role === "aluno"
  ...
  const descriptorsPage = async () => {
    ...
    content().innerHTML = `<div class="page-head"><div><h2>Descritores curriculares</h2><p>Referência institucional para trilhas e avaliações.</p></div><button class="btn btn-primary" id="new">${icon("add")} Novo descritor</button></div><div class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>Código</th><th>Título</th><th>Matéria</th><th>Série / Trimestre</th><th>Status</th><th></th></tr></thead><tbody>${rows.map((x) => `<tr><td><b>${esc(x.codigo)}</b></td><td>${esc(x.titulo)}</td><td>${esc(x.materia_codigo)}</td><td>${x.serie}º · ${x.trimestre}º tri</td><td>${status(x.status === "ativo", x.status, x.status)}</td><td><button class="btn edit" data-id="${x.id}">Editar</button></td></tr>`).join("")}</tbody></table>${rows.length ? "" : empty("Cadastre os descritores oficiais.")}</div></div>`;
    const open = async (x) => {
      const d = await modal(
