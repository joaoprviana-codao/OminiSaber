(() => {
  const groups=[
    ['Gestão acadêmica',['dashboard','turmas','alunos','professores','vinculos']],
  ...
  if(nav){const links=[...nav.querySelectorAll('a')];nav.innerHTML='';groups.forEach(([title,keys])=>{const box=document.createElement('div');box.className='manager-nav-group';box.innerHTML=`<div class="manager-nav-title">${title}</div>`;keys.forEach(key=>{const link=links.find(a=>a.getAttribute('href')?.includes(`/${key}/`)||(key==='dashboard'&&a.getAttribute('href')?.includes('/dashboard/')));if(link)box.append(link)});nav.append(box)});}
  if(document.body.dataset.managerPage==='dashboard'){
    const h1=document.querySelector('.title-wrap h1');
    if(h1){const hour=new Date().getHours();h1.textContent=`${hour<12?'Bom dia':hour<18?'Boa tarde':'Boa noite'}, Gestor`;}
    const actions=document.querySelector('.header-actions');
    if(actions&&!actions.querySelector('.new-account')){const link=document.createElement('a');link.className='btn btn-primary new-account';link.href='../acessos/index.html';link.innerHTML='<span class="material-symbols-rounded">add</span> Nova conta';actions.append(link);}
  }
  ...
  window.renderManagerExecutiveDashboard=async({api,content,route,icon,esc})=>{
    ...
    const coveredCodes=new Set(data.trails.filter(x=>x.publicada&&x.descritor_sedu).map(x=>x.descritor_sedu));
    const covered=data.descriptors.filter(x=>coveredCodes.has(x.codigo)).length;
    const coverage=data.descriptors.length?Math.round(covered/data.descriptors.length*100):0;
    const components=[
      ['Português','portugues_literatura'],['Matemática','matematica'],['Física','fisica'],['Redação','redacao'],['Administração','administracao'],['Informática','informatica']
    ].map(([label,code])=>{const all=data.descriptors.filter(x=>x.materia_codigo===code),done=all.filter(x=>coveredCodes.has(x.codigo));return{label,code,total:all.length,covered:done.length,percent:all.length?Math.round(done.length/all.length*100):0}});
    const priorities=[
      {icon:'group_off',label:'Alunos sem turma',detail:'Necessitam de alocação',count:students.filter(x=>!x.turma_id).length,color:'red',to:'alunos'},
      {icon:'person_alert',label:'Professores sem vínculo',detail:'Necessitam de alocação',count:teachers.filter(x=>!linksByTeacher.has(x.id)).length,color:'orange',to:'vinculos'},
      {icon:'menu_book',label:'Descritores sem conteúdo',detail:'Aguardam publicação',count:data.descriptors.filter(x=>!coveredCodes.has(x.codigo)).length,color:'yellow',to:'descritores'},
      {icon:'lock_reset',label:'Redefinições de acesso',detail:'Solicitações pendentes',count:data.accesses.filter(x=>x.status==='pendente').length,color:'blue',to:'acessos'}
    ];
