...
window.renderPortugueseEssays = async ({ content, data, api, preview, escapeHtml, toast, reload }) => {
  ...
  const studentName = (essay) => essay.perfis?.nome || 'Aluno sem nome cadastrado';
  const initials = (name) => name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'AL';
  const dateTime = (value) => value ? new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(value)) : 'Data não informada';
  const corrected = essays.filter((essay) => essay.status === 'corrigida');
  const pending = essays.filter((essay) => essay.status === 'enviada');
  const scored = corrected.filter((essay) => Number.isFinite(Number(essay.nota)));
  const averageScore = scored.length ? Math.round(scored.reduce((sum, essay) => sum + Number(essay.nota), 0) / scored.length) : null;
  const correctionDurations = corrected.filter((essay) => essay.enviada_em && essay.corrigida_em).map((essay) => new Date(essay.corrigida_em) - new Date(essay.enviada_em)).filter((value) => value >= 0);
  const averageCorrection = correctionDurations.length ? correctionDurations.reduce((sum, value) => sum + value, 0) / correctionDurations.length : null;
  ...
  const deadlines = essays.filter((essay) => essay.enviada_em && essay.propostas_redacao?.prazo);
  const onTime = deadlines.length ? Math.round((deadlines.filter((essay) => new Date(essay.enviada_em) <= new Date(essay.propostas_redacao.prazo)).length / deadlines.length) * 100) : null;
  ...
  if (globalPromptButton) globalPromptButton.onclick = () => promptDialog.showModal();
  const renderEmpty = (icon, title, message) => `<div class="empty-state"><span class="material-symbols-outlined">${icon}</span><h2>${title}</h2><p>${message}</p></div>`;
  const getCompetencies = (essay) => {
    const source = draftByEssay.get(essay.id)?.competencias || essay.avaliacoes_competencias_redacao || [];
    return [1, 2, 3, 4, 5].map((number) => source.find((item) => Number(item.competencia) === number) || { competencia: number, nota: 0, comentario: '' });
  };
  const scoreOptions = (selected) => [0, 40, 80, 120, 160, 200].map((value) => `<option value="${value}" ${Number(selected) === value ? 'selected' : ''}>${value}</option>`).join('');
  ...
  const bindCorrection = () => {
    ...
    list?.addEventListener('click', (event) => { const button = event.target.closest('[data-essay-id]'); if (!button) return; selectedId = button.dataset.essayId; renderCorrection(); });
    const filter = () => { const term = panel.querySelector('[data-essay-search]')?.value.trim().toLowerCase() || ''; const status = panel.querySelector('[data-essay-status]')?.value || 'todos'; panel.querySelectorAll('[data-essay-id]').forEach((item) => { item.hidden = Boolean(term && !item.textContent.toLowerCase().includes(term)) || (status !== 'todos' && item.dataset.status !== status); }); };
    panel.querySelector('[data-essay-search]')?.addEventListener('input', filter);
    panel.querySelector('[data-essay-status]')?.addEventListener('change', filter);
    const form = panel.querySelector('[data-correction-form]');
    if (!form) return;
    const updateTotal = () => { let total = 0; form.querySelectorAll('[name^="competence-"]').forEach((select) => { total += Number(select.value); select.closest('.competence-card').querySelector('[data-competence-value]').textContent = select.value; }); form.querySelector('[data-total-score]').textContent = total; };
    form.querySelectorAll('[name^="competence-"]').forEach((select) => select.addEventListener('change', updateTotal));
    form.addEventListener('submit', async (event) => {
      event.preventDefault(); const submitter = event.submitter; submitter.disabled = true;
      const competencies = [1, 2, 3, 4, 5].map((number) => ({ competencia: number, nota: Number(form.elements[`competence-${number}`].value), comentario: '' }));
      const feedback = form.elements.feedback.value.trim(); const score = competencies.reduce((sum, item) => sum + item.nota, 0);
      try {
        if (submitter.value === 'draft') { const saved = await api.saveEssayCorrectionDraft(form.dataset.essayId, { score, feedback, competencies }); draftByEssay.set(form.dataset.essayId, saved); toast('Rascunho de correção salvo no Supabase.'); }
        else { if (!feedback) throw new Error('Escreva uma devolutiva antes de devolver a redação.'); await api.correctEssay(form.dataset.essayId, { score, feedback, competencies }); toast('Redação corrigida e devolvida ao aluno.'); await reload(); }
      } catch (error) { toast(error.message, 'error'); } finally { submitter.disabled = false; }
    });
  };
  ...
  const renderPrompts = () => {
    ...
    panel.querySelector('[data-new-prompt]')?.addEventListener('click', () => promptDialog.showModal());
  };

  const renderEvolution = () => {
    const allCompetencies = corrected.flatMap((essay) => essay.avaliacoes_competencias_redacao || []);
    panel.innerHTML = `<section class="panel"><div class="panel-heading"><div><p class="eyebrow">Dados consolidados</p><h2>Evolução por competência</h2><p>Médias calculadas somente com redações corrigidas.</p></div></div>${allCompetencies.length ? `<div class="evolution-grid">${[1,2,3,4,5].map((number) => { const values = allCompetencies.filter((item) => Number(item.competencia) === number).map((item) => Number(item.nota)); const average = values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : null; return `<article class="evolution-card"><span>Competência ${number}</span><strong>${average ?? '—'}${average === null ? '' : '/200'}</strong><div class="progress-bar"><i style="width:${average === null ? 0 : average / 2}%"></i></div></article>`; }).join('')}</div>` : renderEmpty('monitoring', 'Ainda não há série histórica', 'As médias serão exibidas após a primeira redação corrigida.')}</section>`;
  };

  const renderView = () => { if (currentView === 'entregas') renderCorrection(); else if (currentView === 'propostas') renderPrompts(); else renderEvolution(); };
  content.querySelectorAll('[data-writing-view]').forEach((button) => button.addEventListener('click', () => { currentView = button.dataset.writingView; content.querySelectorAll('[data-writing-view]').forEach((item) => item.classList.toggle('active', item === button)); renderView(); }));
  content.querySelector('[data-prompt-form]').addEventListener('submit', async (event) => {
    const submitter = event.submitter; if (!submitter || submitter.value === 'cancel') return; event.preventDefault(); submitter.disabled = true; const values = new FormData(event.currentTarget);
    try { await api.createWritingPrompt({ title: values.get('title').trim(), category: values.get('category').trim(), command: values.get('command').trim(), rubric: values.get('rubric').trim(), deadline: values.get('deadline') || null, classId: values.get('classId') || null, published: submitter.value === 'publish', pinned: values.get('pinned') === 'on' }); toast(submitter.value === 'publish' ? 'Proposta publicada para os alunos.' : 'Proposta salva como rascunho.'); promptDialog.close(); await reload(); } catch (error) { toast(error.message, 'error'); } finally { submitter.disabled = false; }
  });
  renderView();
};
