(() => {
  ...
  if (!form || !identifierInput || !passwordInput || !passwordToggle || !loginError || !resendButton || !submitButton || !btnLabel) return;
  ...
  const normalizedError = (error) => `${error?.code || ''} ${error?.message || ''}`.toLowerCase();
  ...
  const presentAuthError = (error) => {
    ...
    if (message.includes('invalid_credentials') || message.includes('invalid login credentials') || message.includes('matrícula não encontrada')) {
    ...
    if (message.includes('could not find the table') || message.includes('schema cache') || message.includes('does not exist')) {
  ...
  passwordToggle.addEventListener('click', () => {
    ...
    passwordToggle.querySelector('.material-symbols-rounded').textContent = isShowing ? 'visibility' : 'visibility_off';
  });
  ...
  document.querySelector('[data-forgot-password]').addEventListener('click', async (event) => {
    ...
    if (!email.includes('@')) {
      showStatus('Digite seu e-mail no campo acima para receber o link de recuperação.');
    ...
    try {
      ...
      showStatus('Se este e-mail estiver cadastrado, você receberá um link para criar uma nova senha.', 'success');
    } catch (error) {
  ...
  resendButton.addEventListener('click', async () => {
    try {
      ...
      const { error } = await window.OminiSaber.resendSignupConfirmation(identifierInput.value);
      if (error) throw error;
      showStatus('Novo e-mail de confirmação enviado. Confira também a pasta de spam.', 'success');
      resendButton.hidden = true;
    } catch (error) {
      showStatus(presentAuthError(error));
    } finally {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (rememberInput.checked) localStorage.setItem('ominisaber-login-email', identifier);
    else localStorage.removeItem('ominisaber-login-email');

    setLoading(true);
    let authenticated = false;
    try {
      if (!window.OminiSaber?.configured) throw new Error('Sistema indisponível: conexão com o Supabase não configurada.');
      const { data, error } = await window.OminiSaber.signIn(identifier, password);
      if (error) throw error;
      if (!data?.session || !data?.user) throw new Error('A autenticação não criou uma sessão válida.');
      authenticated = true;
  ...
  if (!params.has('preview')) {
    window.OminiSaber.getSession()
      .then((session) => session?.user && redirectAuthenticatedUser(session.user.id))
      .catch(async () => { await window.OminiSaber.clearSession().catch(() => {}); });
  }
})();
