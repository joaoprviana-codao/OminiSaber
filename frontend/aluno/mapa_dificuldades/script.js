(() => {
  ...
  const statusFor = (trail) => {
    ...
    const score = total ? Math.round(((trail.concluidas || 0) / total) * 100) : null;
    if (score === null) return { score, className: '', label: 'Sem evidência' };
    if (score >= 80) return { score, className: 'success', label: 'Domínio forte' };
    if (score >= 60) return { score, className: 'warning', label: 'Em evolução' };
  ...
  const renderMap = (code) => {
    ...
    const totalActivities = visible.reduce((sum, trail) => sum + (trail.atividades?.length || 0), 0);
    const totalCompleted = visible.reduce((sum, trail) => sum + Number(trail.concluidas || 0), 0);
    const general = totalActivities ? Math.round((totalCompleted / totalActivities) * 100) : null;
    ...
    if (!visible.length) {
      ...
      elements.priorityList.innerHTML = '<div class="priority-empty"><strong>Nada para priorizar agora</strong><p>Não existem dados publicados para esta matéria.</p></div>';
      return;
    }

    elements.network.style.setProperty('--network-height', `${Math.max(620, Math.ceil(visible.length / 2) * 170)}px`);
    const positions = visible.map((_, index) => getPosition(index, visible.length));
    const lines = positions.map((position) => `<line x1="50%" y1="50%" x2="${position.x}%" y2="${position.y}%"></line>`).join('');
    const nodes = statuses.map((item, index) => {
    ...
    const priority = statuses.filter((item) => item.score !== null && item.score < 80).sort((a, b) => a.score - b.score);
    elements.priorityList.innerHTML = priority.length ? priority.map((item, index) => `<a class="priority-item" href="../modulo_de_trilhas/trilha/index.html?trilha=${encodeURIComponent(item.trail.id)}"><span class="priority-rank">${index + 1}</span><span><strong>${escapeHTML(item.trail.descritor_sedu || item.trail.titulo)}</strong><small>${escapeHTML(item.trail.titulo)} · ${item.trail.atividades.length} etapa(s)</small></span><span class="priority-score">${item.score}%</span></a>`).join('') : '<div class="priority-empty"><strong>Excelente avanço</strong><p>Nenhuma trilha com evidência está abaixo de 80% nesta matéria.</p></div>';
  };
  const load = async () => {
    showState('loading');
    try {
      if (!api()?.configured) throw new Error('A conexão com o Supabase não está configurada.');
      const [profile, catalog] = await Promise.all([api().getProfile(), api().listStudyCatalog()]);
  ...
  elements.select.addEventListener('change', (event) => renderMap(event.target.value));
