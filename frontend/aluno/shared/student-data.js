(() => {
  ...
  const bindActions = () => {
    ...
    document.querySelectorAll('[data-student-theme]').forEach((button) => button.addEventListener('click', () => { localStorage.setItem('edutech-theme', button.dataset.studentTheme); document.documentElement.classList.toggle('dark', button.dataset.studentTheme === 'dark'); notify('Tema atualizado.', 'success'); }));
