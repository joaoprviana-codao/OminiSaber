(() => {
  ...
  menu?.addEventListener('click', () => {
    ...
    menu.setAttribute('aria-expanded', String(body.classList.contains('nav-open')));
  });
  ...
  document.addEventListener('ominisaber:ready', async (event) => {
    ...
    try {
      ...
      document.querySelectorAll('[data-shell-name]').forEach((node) => { node.textContent = name; });
      document.querySelectorAll('[data-shell-avatar]').forEach((node) => {
        node.textContent = profile?.avatar_url ? '' : initials(name);
        if (profile?.avatar_url) node.style.backgroundImage = `url("${String(profile.avatar_url).replace(/"/g, '')}")`;
      });
      document.querySelectorAll('[data-shell-class]').forEach((node) => { node.textContent = profile?.turmas?.serie ? `${profile.turmas.serie}º ano` : 'Ensino Médio'; });
    } catch { /* A página principal mostra o erro da operação relevante. */ }
  });
})();
